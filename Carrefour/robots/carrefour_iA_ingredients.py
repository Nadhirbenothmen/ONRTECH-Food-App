...
from src.countries.france.carrefour.db.carrefour_uploader import get_all_products_from_mongodb_by_category_with_ia_ingredient
...
'''
1- first from args, i should read cateogry name then map it to aisle id 
then search all products from mongo db that has 'categories.id': 27070.
the goal is insead of sending a query to mongo db for each product, lets retrieve the whole 
list of products that belong to that aisle or category
2- result is assigned to a variable then search for each product, not hhaving a ia_ingredient
to search for ia_ingredients from openiA
'''
