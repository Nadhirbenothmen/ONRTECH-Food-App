...
def restore_fields(brut_path, enriched_path):
    ...
    for e in enriched_list:
        ...
        if ean in brut_map:
            ...
            new_ing = brut_map[ean]["ingredients"]
            ...
            new_nut = brut_map[ean]["nutrition"]
            if new_nut is not None and evo.get("nutrition") != new_nut:
                print(f"{os.path.basename(enriched_path)} | EAN {ean}: nutrition restaurée")
                evo["nutrition"] = new_nut
                modified = True
                restored_count += 1
        else:
...
if __name__ == "__main__":
    ...
    print("\n📊 Rapport final:")
    print(f"   Champs restaurés : {total_restored}")
