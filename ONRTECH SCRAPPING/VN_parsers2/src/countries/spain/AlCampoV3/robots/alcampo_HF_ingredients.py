...
CACHE_FILE = "alcampoHF_cache_ingredients.json"
def load_cache():
...
def clean_extra_symbols(text: str) -> str:
    """
    Nettoie une chaîne d'ingrédients en supprimant :
    - les accolades {…}
    - les barres obliques /
    - les caractères superflus isolés
    """
    text = re.sub(r"\{[^}]*\}", "", text)  # supprime contenu entre {}
    text = text.replace("{", "").replace("}", "")  # sécurité supplémentaire
    text = text.replace("/", ", ")  # transforme / en virgule pour séparer correctement
    text = re.sub(r"\s+", " ", text).strip()  # supprime espaces en trop
    return text
...
def process_product(product: dict) -> Product:
    ...
    for evol in product.evolutions or []:
        ...
        if evol.ingredients and len(evol.ingredients) > 2:
            ...
            ingredients = clean_extra_symbols(ingredients)
