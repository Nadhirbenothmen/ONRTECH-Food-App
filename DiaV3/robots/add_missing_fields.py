...
def compute_nutriscore(nutrition: dict) -> str:
    ...
    try:
        ...
        salt_val = minerals.get("salt", 0) or 0

        # 🚩 Correction automatique : si salt < 5 → considéré comme grammes, on convertit en mg
        if salt_val < 5:
            salt_mg = salt_val * 1000
            minerals["salt"] = salt_mg   # ⚠️ écrase la valeur dans le JSON
            print(f"⚠️ Correction sel: {salt_val} g → {salt_mg} mg")
        else:
            salt_mg = salt_val
            minerals["salt"] = salt_mg   # uniformise pour toujours stocker en mg

        # conversion en grammes pour le calcul des points
        salt = salt_mg / 1000.0  
...
def clean_brand(candidate: str) -> str:
    """
    ...
    parts = [p for p in parts if p.lower() not in FORBIDDEN_SUFFIX]
...
def extract_brand_from_title(title: str) -> str:
    ...
    if match:
        return clean_brand(match[-1].strip())
    ...
    try:
        ...
        for product in products:
            ...
            if mesure_unit_for_packaging and mesure_unit_for_price_per_unit:
                ...
                # === Recalcul systématique du Nutri-Score ===
                for evo in evolutions:
                    ns_old = evo.get("nutri_Score")
                    ns_calc = compute_nutriscore(evo.get("nutrition", {}))
                    if ns_calc and ns_calc not in ["N/A", None]:
                        evo["nutri_Score"] = ns_calc
                        if ns_old != ns_calc:
                            print(f"🥗 Nutri-Score recalculé: {ns_old} → {ns_calc} (EAN={product.get('ean')})")
                            modified = True
