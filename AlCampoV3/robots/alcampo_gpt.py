...
# --- Cache IA ---
CACHE_FILE = "alcampoGPT_cache_ingredients.json"
def load_cache():
...
def extract_base_ingredients_es(text: str) -> str:
    ...
    # 🛠 Corrections manuelles pour éviter les coupures incorrectes
    text = text.replace("proteínas hidrolizadas parcial, enzimáticamente de suero de leche", "proteínas hidrolizadas de suero de leche")
    text = text.replace("aceite de la microalga schizochytrium sp", "aceite de microalga schizochytrium sp")
    ...
    remove_if_contains = [
        ...
        "enzimas", "fermentos", "cultivos", "nucleótidos", "aminoácidos",
        "aceite de pescado", "bifidobacterias", "lactobacilos", "l-carnitina",
        "cloruro de colina", "taurina", "inositol", "mezcla de tocoferoles", "palmitato de ascorbilo",

        # Oligosaccharides et autres molécules complexes à exclure
        "fucosil-lactosa", "difucosil-lactosa", "lacto-n-tetraosa", "sialilactosa", "3'-sl", "6'-sl",
        "oligosacárido", "oligosacáridos", "nucleótido", "nucleótidos"
    ]
    ...
    for part in parts:
        ...
        if any(bad_kw in part for bad_kw in remove_if_contains):
            continue
        if len(part.split()) > 6:  # ← Optionnel : augmenter si certains bons ingrédients sont longs
            continue
