...
def is_illogical(val):
    ...
    for ing in val:
        ...
        # 🚨 Ajout : si c'est un entier pur (EAN), c'est illogique
        if ing_low.isdigit():
            return True
...
def enrich_product(prod, cache):
    ...
    if "ingredients" in cached and cached["ingredients"]:
        evol["ingredients"] = cached["ingredients"]
        changed = True

    # Toujours écraser ingredients_ia si dispo dans le cache
    if "ingredients_ia" in cached and cached["ingredients_ia"]:
        evol["ingredients_ia"] = cached["ingredients_ia"]
        changed = True


    # Toujours écraser ingredients_clean si dispo dans le cache
    if "ingredients_clean" in cached:
