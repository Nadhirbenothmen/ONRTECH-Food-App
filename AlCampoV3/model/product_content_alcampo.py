...
class ProductContentAlcampo(object): 

    HALAL_PRESENCE: Final [List[str]] = ["halal", "Halal" , "HALAL", "kosher"]

    BIO_PRESENCE: Final [List[str]] = ["ECOLÓGICO", "eco","biológicas", "ecológico" ,"agricultura ecológica","ecológica" ,"Organic", "BIO" , 
                                       "ecológicos" , "ecológicas" , "biológica" , "Eco", "100% natural", "100 % naturales"]
    
    VEGAN_PRESENCE: Final [List[str]] = ["vegano", "Vegetánea", "végétalien"  , "Vegano" , "100% vegetal."]
    ...
    SPANISH_PRESENCE:Final [List[str]] = ["España" , "Origen de la leche España","ganaderos españoles", "ORIGEN DE LA LECHE: ESPAÑA", "de origen español" 
                                         ,"ORIGEN: ESPAÑA","Origen Nacional" ,"País de Ordeño: España","receta española", "Proviene de la agricultura española",
                                           "País de transformación: España" ,"agricultura española "]
    
    SPANICH_ABSENCE:Final [List[str]] = [ "HOLANDA", "Francia", "Harina acondicionada de trigo", "República Checa"]


    LACTOSE_PRESENCE:Final [List[str]] = ["lactosa", "lácticos","no contienen lactosa", "no contiene lactosa",
                                           "LÁCTEOS" ,"CONTIENE LACTOBACILLUS CASEI","aroma (contiene lactosa)","LECHE Y SUS DERIVADOS (INCLUIDA LA LACTOSA)",]
    
    LACTOSE_ABSENCE:Final [List[str]] = [" no contien lactosa" ,"LACTASA" , "leche pasteurizada sin lactosa", "elaborado sin lactosa" ,"sin lacosa", "Sin lácteos","ni lactosa"
                                         "Leche en polvo desnatada sin lactosa","Naturalmente sin lactosa", "Sin lactosa","lactasa" ,"enzima lactasa"]
    
    GLUTEN_PRESENCE: Final [List[str]] = [ "gluten de trigo", "PASTA ALIMENTICIA DE SÉMOLA DE TRIGO DURO (GLUTEN)","Semola de trigo duro (100%)(gluten)",
                                           "pan rallado (contiene gluten)","Harina integral de avena (contiene gluten)", 
                                           ...
                                           "Puede contener trazas de cereales que contienen gluten" , "Harina de TRIGO (GLUTEN)" , "harina de avena (gluten)"] 
    
    GLUTEN_ABSENCE: Final [List[str]] = ["ni gluten" ,"Sin gluten", "sin gluten 100%" , "sin gluten","pan rallado sin gluten", "no contiene gluten" , "ni gluten", "No contiene gluten"]

    FAT_ABSENCE:Final [List[str]] = ["ni grasa ", "sin grasas añadidas", "sin grasas" ,"Sin grasas añadidas" ,"Sin grasa añadida" ,"Sin grasa" ,"Sin grasas"]

    SUGAR_ABSENCE:Final [List[str]] = ["sin azúcar","0% azucar" "no tiene azúcares añadidos","ni azúcares añadidos", "Sin azucares añadidos" , "sin azúcar añadido" ,"sin azúcares añadidos" ,"Sin azúcares añadidos" ,"Sin azúcar añadido" , "sin azucares"]
    ...
    GMO_ABSENCE:Final [List[str]] = ["0% Materia Grasa", "sin OGM", "sin OGM" ,"Sin OGM" ,"Sin organismos modificados genéticamente" ,"Sin organismos genéticamente modificados", "0% materia grasa"]
    ...
    def get_packaging_info(cls, product_pricing_unit: Optional[str]) -> tuple[str | None, str | None, float] | None:
        ...
        if "kilogramo" in product_pricing_unit or "kg" in product_pricing_unit:
            matter = ProductType.SUBSTANCE.name
            unit_of_mesure = "Kg"
            product_pricing_unit = product_pricing_unit.replace(
                "kilogramo", "").replace("kg", "").replace("g", "").replace("gr", "")
    ...
    def get_label_and_origin(driver: webdriver) -> Tuple[Optional['Label'], Optional[str]]:
        ...
        try:
            # Trouver le bloc "Atributos del producto"
            ...
            icon_blocks = attributes_block.find_elements(By.CSS_SELECTOR, "div.salt-p--1")
            ...
            for block in icon_blocks:
                try:
                    ...
                    # SVG → data-icon
                    try:
                        svg = block.find_element(By.TAG_NAME, "svg")
                        icon_id = svg.get_attribute("data-icon") or ""
                    except:
                        pass  # SVG optionnel
                    ...
                    # 🔁 Matching data-icon ou texte
                    if "lactose_free" in icon_id or "sin lactosa" in label_text:
                        label.lactose = False
                    elif "cooled" in icon_id or "refrigerado" in label_text:
                        label.cooled = True
                    elif "vegan" in icon_id or "vegano" in label_text:
                        label.vegan = True
                    elif "eco" in icon_id or "biológico" in label_text:
                        label.bio = True
                    elif "gluten" in icon_id or "sin gluten" in label_text:
                        label.gluten_free = True
                    elif "gmofree" in icon_id or "sin transgénicos" in label_text:
                        label.gmo_free = True
                    elif "froozen" in icon_id or "frozen" in icon_id or "congelado" in label_text:
                        label.frozen = True
                    elif "pregnancy" in icon_id or "embarazo" in label_text:
                        label.pregnancy_unsafe = True
                    elif "vegetarian" in icon_id or "vegetariano" in label_text or "fruitslegumesfr" in icon_id:
                        ...
                        spanish_origin = "espana"
                    elif "fatfree" in icon_id or "sin grasa" in label_text:
                        label.fat_free = True
                    elif "addedsugarfree" in icon_id or "sin azúcar añadido" in label_text:
                        label.sugar_added_free = True

                except Exception as e:
                    print(f"⚠️ Erreur lors du traitement d'un label : {e}")
                    continue

        except Exception as e:
            print(f"⚠️ Erreur globale lors de l'extraction des labels : {e}")
...
class NutritionAlcampo(Nutrition):
    @classmethod
    @override
    def convert_dict_to_nutrition_object(cls, nutritions: dict | None) -> Nutrition | None:
        ...
        for key, value in nutritions.items():
            ...
            # energy
            if are_string_exist_in_list(key, NutritionContentAlcampo.ENERGY_VALUE_KJ_FINAL):
                value = remove_substring_and_clean(value, "kj")
                energies.kj = convert_to_float(value)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.ENERGY_VALUE_KCAL_FINAL):
                value = remove_substring_and_clean(value, "kcal")
                energies.kcal = convert_to_float(value)

            # macronutrients
            elif are_string_exist_in_list(key, NutritionContentAlcampo.FATS_FINAL):
                fats.fats = convert_to_target_unit(value, fats_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.SATURATES_FINAL):
                fats.saturates = convert_to_target_unit(value, fats_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.CARBOHYDRATES_FINAL):
                carbohydrates.carbohydrates = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.WITH_SUGAR_FINAL):
                carbohydrates.of_which_sugars = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.SUGARS_FINAL):
                carbohydrates.sugars = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.DIETARY_FIBER_FINAL):
                carbohydrates.dietary_fiber = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.STARCH_FINAL):
                carbohydrates.starch = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_strings_equal(key, NutritionContentAlcampo.LACTOSE_FINAL, 90):
                carbohydrates.lactose = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.GAL_FINAL):
                carbohydrates.gal = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.FIBER_FINAL):
                carbohydrates.fiber = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.POLYOLS_FINAL):
                carbohydrates.polyols = convert_to_target_unit(value, carbohydrates_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.PROTEINES_FINAL):
                proteins.proteins = convert_to_target_unit(value, proteins_measurement)

            # Minerals
            elif are_strings_equal(key, NutritionContentAlcampo.SODIUM_FINAL, 90):
                minerals.sodium = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.SALT_FINAL):
                minerals.salt = convert_to_target_unit(value, minerals_measurement)
            elif are_strings_equal(key, NutritionContentAlcampo.CALCIUM_FINAL, 90):
                minerals.calcium = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.MANAGANESE_FINAL):
                minerals.manganese = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.MAGNESIUM_FINAL):
                minerals.magnesium = convert_to_target_unit(value, minerals_measurement)
            elif are_strings_equal(key, NutritionContentAlcampo.POTASSIUM_FINAL, 90):
                minerals.potassium = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.CHROMIUM_FINAL):
                minerals.chromium = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.PHOSPHORUS_FINAL):
                minerals.phosphorus = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.IRON_FINAL):
                minerals.iron = convert_to_target_unit(value, minerals_measurement)
            elif are_strings_equal(key, NutritionContentAlcampo.ZINC_FINAL, 90):
                minerals.zinc = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.IODINE_FINAL):
                minerals.iodine = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.CHLORIDE_FINAL):
                minerals.chloride = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.COPPER_FINAL):
                minerals.copper = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.FLUORIDE_FINAL):
                minerals.fluoride = convert_to_target_unit(value, minerals_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.SELENIUM_FINAL):
                minerals.selenium = convert_to_target_unit(value, minerals_measurement)

            # FattyAcids
            elif are_string_exist_in_list(key, NutritionContentAlcampo.LINOLEIC_ACID_FINAL):
                fatty_acids.linoleic_acid = convert_to_target_unit(value, fatty_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.ALPHA_LINOLENIC_ACID_FINAL):
                fatty_acids.alpha_linolenic_acid = convert_to_target_unit(value, fatty_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.OMEGA_3_FINAL):
                fatty_acids.omega_3 = convert_to_target_unit(value, fatty_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.SATURATED_FATTY_ACIDS_FINAL):
                fatty_acids.saturated_fatty_acids = convert_to_target_unit(value, fatty_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.MONOUNSATURATED_FATTY_ACIDS_FINAL):
                fatty_acids.monounsaturated_fatty_acids = convert_to_target_unit(value, fatty_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.INCLUDING_MONOUNSATURATED_FATTY_ACIDS_FINAL):
                fatty_acids.including_monounsaturated_fatty_acids = convert_to_target_unit(value, fatty_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.INCLUDING_POLYUNSATURATED_FATTY_ACIDS_FINAL):
            ...
            elif are_strings_equal(key, NutritionContentAlcampo.CHOLINE_FINAL, 90):
                vitaminLikes.choline = convert_to_target_unit(value, vitaminLikes_measurement)
            elif are_strings_equal(key, NutritionContentAlcampo.INOSITOL_FINAL, 90):
                vitaminLikes.inositol = convert_to_target_unit(value, vitaminLikes_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.L_CARNITINE_FINAL):
                vitaminLikes.lcarnitine = convert_to_target_unit(value, vitaminLikes_measurement)
            elif are_strings_equal(key, NutritionContentAlcampo.FOS_FINAL, 90):
                vitaminLikes.fos = convert_to_target_unit(value, vitaminLikes_measurement)
            elif are_strings_equal(key, NutritionContentAlcampo.GOS_FINAL, 90):
            ...
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_B1):
                vitamins.vitamin_b1 = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_B2):
                vitamins.vitamin_b2 = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_B3):
                vitamins.vitamin_b3 = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_B5):
                vitamins.vitamin_b5 = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_B6):
                vitamins.vitamin_b6 = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_B7):
                vitamins.vitamin_b7 = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_B9):
                vitamins.vitamin_b9 = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_B12):
                vitamins.vitamin_b12 = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_C):
                vitamins.vitamin_c = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_A):
                vitamins.vitamin_a = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_E):
                vitamins.vitamin_e = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_K):
                vitamins.vitamin_k = convert_to_target_unit(value, vitamins_measurement)
            elif are_string_exist_in_list(key, NutritionContentAlcampo.VITAMINE_D):
    ...
    def convert_to_nested_json(input_dict: Dict[str, bool]) -> List[Label]:
        ...
        return ProductContentAlcampo.keep_first_element(unique_labels)
